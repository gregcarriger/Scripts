...
$blob_service_sas_URL = "https://abcdefghijklmnopqrstuvwxyz.blob.ingestion.blobl4.protection.office365.com/ingestiondata?sv=2015-04-05&sr=c&si=IngestionSasForAzCopy1234567890123456789&sig=12345678901234567890123456789&se=2021-11-04T18"
...
$pst_files | ForEach-Object {
	...
	If ($_.Length -gt 19000000000)
		...
		write-output "$($_.Name) renamed to $($_.Name).toobig"
	}
	Elseif ($_.{FullName} -replace $source_pst_location_fixed , "" )
		...
		write-output "$($_.Name) renamed to $($_.TargetRootFolder)"
	}
	Else
		{
	}
...
# upload .pst files
$pst_files | ForEach-Object {
	& $azcopy_exe  /Source:$([string]($_.NewFullName).Substring(0, $_.NewFullName.lastIndexOf('\'))) /Dest:$($blob_service_sas_URL) /Pattern:$($_.name) /v:$($azcopy_log_directory)$($_.Name)-azcopy.log /y
#	Rename-Item -Path $_.NewFullName -NewName "$($_.Name).uploaded
}
$pst_files | export-csv -NoTypeInformation $azcopy_log_directory"Pst_files_debug.csv"
